...
使用声明：⚠️仅供参考，🈲转载与售卖！
额外说明：收入猫可以单个写，又有现成的合集(写的挺好挺完整)，所以就不写了偷偷懒，自己碰到这类软件会添加上面没有的软件。仅作自用备份，不会进行任何商业用途！

**************************************

[rewrite_local]
^https:\/\/(api\.revenuecat|isi\.csan\.goodnotes)\.com\/.+\/(receipts$|subscribers\/?(.*?)*$|offers$) url script-response-body https://raw.githubusercontent.com/Mike-offers/Rewrite/refs/heads/master/QuantumultX/Revenuecat.js
^https:\/\/(api\.revenuecat|isi\.csan\.goodnotes)\.com\/.+\/(receipts$|subscribers\/?(.*?)*$|offers$) url script-request-header https://raw.githubusercontent.com/Mike-offers/Rewrite/refs/heads/master/QuantumultX/Revenuecat.js
...
const list = {
  ...
  'reader': { name: 'com.valo.reader.vip1.forever', id: 'com.valo.reader.vip1.forever', cm: 'sja' },  //读不舍手
